...
| 6 | MCP server exposes ten tools | README, `mcp/__init__.py` docstring | `src/mini_omega_lock/mcp/server.py` `@mcp_app.tool()` decorators | `python scripts/generate_readme_claims.py --check` | `generated` |
...
| 28 | No `omega_lock.preflight` API is cited (it does not exist) | trust model / coupling-safety (M5) | `src/mini_omega_lock/probes.py` docstring + `scripts/check_repo_consistency.py::check_no_stale_omega_lock_preflight_citation` | `python scripts/check_repo_consistency.py` (fails if the string reappears in `src/`) | `command-backed` |
| 29 | `preflight` CLI exits non-zero on unmeasured fields | README "What's new", trust model | `src/mini_omega_lock/cli.py` + `tests/test_cli.py` | `python -m pytest -q tests/test_cli.py` | `source-backed` |
| 30 | `silent_degradation_detected` flips True on a parsed=None probe | README "What's new", trust model | `src/mini_omega_lock/probes.py::probe_strict_schema` + `tests/test_probes.py::test_probe_strict_schema_sets_silent_degradation_on_parsed_none` + `benchmarks/golden_cases/strict_schema_failure.json` | `python -m pytest -q -k silent_degradation`; `python scripts/run_golden_cases.py --check` | `artifact-backed` |
